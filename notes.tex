...
\begin{document}
    ...
    \chapter{What is Learning?}
        \section{Learning to classify}
            ...
            For instance: we might have $\sdia{cow-a},
            ...
            $\Uu \in (\Xx\times\Yy)^N$ drawn independently from $\Dd$.  So
            ``learning to classify'' means mapping $\Uu$s to $f$s.  In
        ...
        \section{When is an algorithm good?}
            An algorithm $\Ll$ is good when $\Ll(\Uu)$ accurately classifies
            images freshly drawn from $\Dd$.  Notice that this is different
            from merely asking that $\Ll(\Uu)$ accurately classifies the
            elements of $\Uu$!  To notate this difference, we define the
            ...
            $$
                \Eout(f) = \PP_{(x,y)\sim\Dd} \left[
                           ...
                           \right]
                ~~~~~~~~~~
                \Ein(f) = \PP_{(x,y)\sim\Uu} \left[
                              f(x) \neq y
                          \right]
            $$
            Likewise, the out- and in-errors of a algorithm are
            $
                \Ee_{\cdots}(\Ll) = \Ee_{\cdots}(\Ll(\Uu)) 
            $ where $\Uu\sim\Dd$.

            We hope $\Eout$ is low, but we don't know
            $\Eout$: from $\Uu$, we can directly calculate only
            $\Ein$.  Intuiting that
            $ 
                \Egap = \Eout - \Ein
            $ 
            ...
            This $\Ll$ will work when $\Egap$ is small.
            But is $\Egap$ actually small?  In particular, if
            $\Ll(\Uu)$ accurately classifies its training samples, will it also
            ...
            Sometimes, the answer is \textbf{yes}.
                ...
                $\Ll(\Uu)$ and $\Uu$ are independent, and we may apply the law
            ...
            Other times, the answer is \textbf{no}.
                ...
                sequence $\Uu$, many candidates will perfectly classify the
                training samples.  Say that $\Ll$ breaks ties 
                by settling on
                $$
                    f(x) = \text{say Cow if $(x,\text{Cow}) \in \Uu$;
                            otherwise, say Dog}
                $$
    ...
    \chapter{Is Learning Possible?}
        ...
        \section{Multiple candidates}
            ...
            parameters, each represented by $32$ bits.  Since there are
            $2^{32}$ values for each parameter, $H \leq 2^{32\cdot 10^4}$.  If
            we train on $10^7$ images, then with probability $99\%=1-10^{-2}$,
    ...
    \chapter{Linear classification}
        ...
        \section{Optimization}
            ...
            $\Uu$?  Since the jaggedness $d$ is finite, it makes sense to
            seek an $f$ with low training error:
            $$
                N \cdot \Ein(\theta) =
                    \sum_{(x,\text{Cow})\in \Uu}
                    \begin{cases} 
                        1       &       \text{if}~\theta(x) < 0 \\
                        0       &       \text{otherwise}
                    \end{cases}
                    + 
                    \sum_{(x,\text{Dog})\in \Uu}
                    \begin{cases} 
                        1       &       \text{if}~\theta(x) > 0 \\
                        0       &       \text{otherwise}
                    \end{cases}
            ...
            \begin{align*}
                ...
                &=
                    \epsilon\left(
                        +
                        \sum_{\substack{(x,\text{Cow})\in \Uu \\ \text{and}~\theta(x)<+1}} x
                        -
                        \sum_{\substack{(x,\text{Dog})\in \Uu \\ \text{and}~\theta(x)>-1}} x
                    \right)
    ...
    \chapter{Example: Classifying Clothing}
        \section{Featurizing and learning}
            ...
            \begin{marginfigure}
                \centering
                \includegraphics[height=12cm]{fashion-mnist-cropped}
                \caption{\emph{
                }}
            \end{marginfigure}
...
%            linear function $\epsilon$, then $\exp((\theta+\epsilon))(x)$  
    ...
    \chapter{Mutual Information}
        \section{Structure in $\Dd$ and $\Ll$}
            Our gap bound so far invokes $H(n)$, which counts the worst-case
            size of $\Hh_\Uu$ over all training sets of size $n$.  However, 
            what if the distribution $\Dd$ is concentrated around ``easy''
            data so that most of the time, $|\Hh_\Uu|$ is much smaller than
            $H(n)$?  For example, what if there's only a $\delta/2$ chance that
            $\Uu \sim \Dd^n$ has $|\Hh_\Uu| \geq H_{\delta/2}(n)$?  Then with
            chance at least $1-\delta/2-\delta/2$, we have:
            $$
                \Egap
                \leq
                \sqrt{\frac{16 \log(2H_{\delta/2}(2N)/(\delta/2))}{N}}
            $$

            %More generally, if we know that whenever $\Uu\in\Ee\subseteq\Dd^N$
            %then $\Ll(\Uu) \in \Kk \subseteq\Hh$, and that $\Ee$ occurs with
            %probability $1-\rho$, then 

        \section{Structure in}
